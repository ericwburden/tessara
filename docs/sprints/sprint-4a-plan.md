...
- Publish route: use the explicit publish endpoint as the only publish flow: `POST /api/admin/components/{component_id}/versions/{version_id}/publish`. Create/update version payloads save drafts only and do not accept a `publish` boolean. Publish locks the component and target draft, then loads, authorizes, and validates the locked draft inside the transaction before superseding prior published versions.
