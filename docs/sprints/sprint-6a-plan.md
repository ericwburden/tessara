...
The working-tree implementation has advanced through steps 1–11. Step 12 is
the active focus: execute the ordered upgraded-data, release-browser, and
fresh-install gates; fix implementation defects without weakening frozen
proof; then record closeout evidence against the closing commit. A working-tree
implementation is not considered complete merely because focused tests pass.
