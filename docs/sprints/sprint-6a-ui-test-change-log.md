...
Status: reconciled on 2026-07-21. Approved Rust, Playwright, smoke, UAT, and manifest identities were replaced only where the schema-v2 composition, direct Admin routes, Module UX, provenance, assignment-date contract, or fresh-sprint closeout policy superseded them. Timeouts, retries, unrelated assertions, and descriptor fixtures remain unchanged.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-07-17 | `crates/tessara-api/src/users/dto.rs` — `role_summary_only_exposes_assignment_time_in_account_context`; `crates/tessara-web/src/features/administration/components/user_forms/account.rs` — `account_role_assignment_explains_global_module_and_scoped_role_composition` | Existing assignment read-model proof extended | Asset 15 approval and subsequent UI-gap review: show authoritative role Scope alongside `Assigned on` with concomitant narrow evidence | Assignment rows could distinguish dates but could not explain why an installation-global Module role coexists with a Scope-aware product role | Yes, additive readback only | Existing assignment-time, separate-role guidance, and no-authorization-change assertions remain; additive assertions require the serialized `installation_global` scope and its visible `Installation-global` label | Product approval recorded in review; this implementation commit |
...
| 2026-07-21 | `crates/tessara-api/migrations/001_baseline.sql`; `crates/tessara-api/tests/sprint_6a_populated_upgrade.rs` — retired `populated_sprint_5a_upgrade_preserves...` helper replaced by executable `fresh_startup_and_seed_assignment_lock_order_use_a_separate_database`; deployment evidence scripts | Fresh-sprint closeout reconciliation | Project policy approved at closeout: destroy prior-sprint databases, squash all current schema into one baseline migration, and start the next sprint from a freshly seeded database | The migration-1–4 ledger, populated-upgrade fixture, and upgraded/rollback acceptance gates contradict the approved lifecycle policy | Yes, closeout evidence is fresh-baseline only | The executable replacement proves the sole migration ledger, immutable baseline checksum/schema, seed, and lock order on a separate fresh database; smoke/UAT and browser evidence require the same state. Historical dashboard preflight behavior and pre-squash helpers remain forensic fixtures, not accepted lifecycle proof. | Product approval recorded 2026-07-21; this closeout commit |

## Closeout Reconciliation

### Approved reviewer reconciliation — 2026-07-21

The following replacements are approved. They preserve the no-skips/no-retries
policy and add behavior proof; neither makes a failing expectation disappear.

| Exact identity | Current failure | Approved equal-or-stronger reconciliation | Reviewer decision |
| --- | --- | --- | --- |
| `end2end/tests/modules.spec.ts` — `Sprint 6A Module Management › global read exposes protected Module Management without an aggregate Administration item and remains read-only` | The identity requires the literal eligibility text `Any of` in the protected Module Management row. That copy was deliberately removed during the approved compact table/lock-only treatment. | Replace the retired text assertion with semantic protected-placement proof: `core.admin.modules` is present in Admin, identifies Module Management, exposes the canonical protected lock treatment, grants the global reader discovery/read-only access, and continues to reject the navigation-policy write with `modules_manage_navigation_global_required`. Retain the existing direct-route, no-aggregate-Administration, descriptor, and denied-write assertions. | Approved. |
| `end2end/tests/permissions.spec.ts` — `admin can create a role and load the roles route` | The identity selected scope-aware `forms:read`, then expected installation-global `modules:read` to remain selectable in the same role. | Use two fresh-picker branches: selecting `forms:read` hides `modules:read`; selecting `modules:read` hides `forms:read`. Keep `admin:all` visibly available as the sole exception and retain separate-role assignment proof, so a user can receive a scoped product role plus a global Module role without an ordinary mixed role. | Approved. |
