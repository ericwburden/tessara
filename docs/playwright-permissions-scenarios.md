...
- Sprint 4A component authoring coverage lives in `end2end/tests/components.spec.ts`: admin creates a component with an atomic first draft, validates unsaved native authoring config and renders field-path findings, upserts the one draft, validates the public aggregate function set (`count`, `count_values`, `count_distinct`, `sum`, `avg`, `min`, `max`), publishes explicitly, executes current and version-specific published aggregate table endpoints with configured source pre-filtering, metric post-filtering, metric-output runtime filtering/sorting, creates a replacement detail draft and verifies the viewer and public reader detail stay on the published aggregate until publish, verifies draft-only components are absent from the public list, publishes the draft from the native publish route, verifies supersede behavior, verifies published/superseded versions are immutable, rejects draft component-version dashboard placement and proves legacy draft placements are ignored by dashboard readers, exercises server-driven detail table search plus `equals`, `not_equals`, `not_contains`, `is_empty`, `is_not_null`, aggregate `between`, sorting, pagination, visible columns, and unknown visible-column validation, and opens the native list/detail/edit/viewer routes over a dataset major-line binding. `crates/tessara-web-components` also covers structured publish-validation findings with field paths plus the native viewer query builder, full filter operator query encoding, and checkbox column-picker behavior so visible-column state remains server-driven without relying on a raw comma-separated control.
...
- Future component coverage should add full browser-native authoring interactions for advanced filters, sorting, pagination, and aggregate post-filters as those controls become richer.
