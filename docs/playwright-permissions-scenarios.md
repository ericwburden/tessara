...
| Shell navigation | Scoped manager can see allowed configured product destinations such as Forms and Responses. A global module reader sees protected Module Management in `Admin`; an administrator sees the four direct Core Admin destinations. | Scoped-only module authority, product-only authority, and no authority do not expose Module Management, including shell loading/failure fallback. The removed aggregate Administration item is never projected. |
| Module Management authority | Global read loads directory/detail/policy read surfaces with no enabled mutation affordance; global manage implies read and can create a custom group, move an optional destination across groups, and save atomically; `admin:all` retains full authority. | Scoped-only, product-only, no-access, and anonymous actors receive the exact restricted/redirect states; direct writes require global manage; protected destinations cannot be hidden or moved out of their required group, and non-empty custom groups cannot be deleted. |
| Module Management parity and cleanup | Inventory/detail IDs, source digests, complete group/placement policy values, desktop/mobile order, SSR/no-JavaScript content, hydration, and API projections agree across the five executable scenarios. | Unknown detail, unavailable/failure states, bridge requests, console/hydration errors, protected-placement changes, revision conflicts, and teardown policy drift are rejected or absent. Original groups and placements are restored even when a test fails. |
