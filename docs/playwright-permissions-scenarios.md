...
| Dataset revisions | Admin and in-scope dataset managers can review draft revisions and publish them; read-only scoped users can read published revisions only. | Read-only scoped users and out-of-scope dataset managers cannot read or publish draft revisions; revision history omits drafts for read-only users. |
