...
| --- | --- | --- | --- |
| Form node attachment | `/forms/:form_id/edit` | Form attachment is now represented through generated single-form workflows and normal workflow assignments, not direct form-node records. "Assign Form" is product shorthand, not a distinct data model. | Continue refining the shortcut UX so form-first actions feel simple while still presenting the generated workflow assignment surface. |
| Assignment source of truth | `/forms/:form_id/edit`, `/workflows/assignments` | `workflow_assignments` are now the single source of truth. Generated single-form workflows are visible product shortcuts, not hidden direct assignment records. | Future work should improve labeling/filtering for generated workflows and add richer step metadata before attempting cross-step data passing. |
| Form scope semantics | `/forms/new`, `/forms/:form_id/edit` | Current form payloads still include legacy `scope_node_type_id`, but the product direction is to stop treating forms as intrinsically scoped to a node type. Forms define response fields and validation; workflow steps define target/context semantics. | Remove explicit form node-type scope after generated single-form workflow creation has a replacement way to choose the workflow node type. Workflow-step metadata should eventually support branching, target resolution, prefills, hidden/locked carried-forward values, and data mappings from prior step responses. |
