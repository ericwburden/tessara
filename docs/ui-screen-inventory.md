...
| Workflow version authoring | `form_version_id`, `title` | `/workflows/new`, `/workflows/:workflow_id/edit` | Do not expose the single-form workflow-version fallback in reset authoring; users author explicit ordered steps, and each step selects its own form version. | Review `CreateWorkflowVersionRequest.form_version_id`, `CreateWorkflowVersionRequest.title`, fallback step creation in workflow handlers, and `workflow_versions.form_version_id` once workflow routes are fully step-first. |
