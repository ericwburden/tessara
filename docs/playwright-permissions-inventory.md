...
| `end2end/tests/modules.spec.ts` | Five Sprint 6A Module Management scenarios covering shell loading/failure, directory/detail parity, policy mutation, desktop/mobile navigation, no-JavaScript SSR, and bridge-request exclusion | Global `modules:read`, manage-implies-read, scoped-only fail-closed behavior, product-only/no-access denial, `admin:all`, required Module Management placement, reader versus manager controls, exact schema-v2 GET/PUT outcomes, and failure-safe policy restoration. |
...
| Module Management | Covered by five executable scenarios in `modules.spec.ts`; the durable acceptance manifest contains 60 tests across 7 files | Keep the global-read/manage/scoped/product-only/no-access/`admin:all` matrix, required `core.admin.modules` placement, reader/manager controls, complete group/destination protection, desktop/mobile ordering, SSR/no-JS parity, seven-descriptor human/machine detail parity, and policy-restoring teardown current as the control plane evolves. Runtime/Module Instance scenarios begin in Sprint 6B. |
