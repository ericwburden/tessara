...
| Shell Context signature and field validation | Contract payload/key validation plus module-local request decoding and verifier construction | Dashboard and Scoped Records | Pure wire validation belongs in contract; base64 header extraction, purpose-bound verifier construction, and correlation extraction repeat | Keep wire types and payload validation in contract; move composite verification and request/runtime adapters to `tessara-module-runtime` |
...
| Root authenticated shell | `crates/tessara-web/src/ui/shell/*`, `AppShell`, root auth guards, root navigation state | Core and Dashboard through root `tessara-web` | Root shell is intentionally coupled to Core routing/session/account actions; Dashboard's dependency is the Sprint 6E defect | Retain the Core shell in root web; module UI owns a separate normalized complete-document frame and shared primitives/tokens, while Sprint 6E removes Dashboard's root dependency |
| Shared UI primitives and placement mechanics | `crates/tessara-web-ui`; generic grid geometry historically in `tessara-core` | Root and feature web crates, including Dashboard and Forms | Inventory proved placement DOM mechanics are shared while product sizing/workflow policies are not | Move generic grid geometry to `tessara-module-contract`, move primitives and placement DOM mechanics to `tessara-module-ui`, retain Dashboard/Form adapters with their product owners, update consumers, and delete `tessara-web-ui` |
...
2. `tessara-web-ui` is not fully independent because
   `placement_editor.rs` imports generic grid operations from the historically
   named `tessara-core` crate. Dashboard and Forms both consume those
   mechanics. Generic geometry moves to contract, shared DOM mechanics move to
   module UI, product adapters remain product-owned, and the old crate is
   deleted.
