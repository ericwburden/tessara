...
The left navigation is computed from permanent Core destinations plus module contributions, administrator visibility and ordering policy, contribution-specific lifecycle requirements, and the user's applicable scope-bound Authorization Grants. Product destinations normally require an enabled instance. Administration, configuration, and diagnostics destinations may remain visible for an installed but disabled, unconfigured, or unhealthy instance so administrators can recover it. Hiding an item does not disable the module or authorize a user. Route and API enforcement remains mandatory inside the owner.
