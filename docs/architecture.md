...
The left navigation is computed from capability-filtered permanent Core destinations plus eligible module contributions, administrator visibility and ordering policy for those mutable contributions, contribution-specific lifecycle requirements, and the user's applicable scope-bound Authorization Grants. Permanent Core destinations are policy-immutable: contribution policy cannot hide, reorder, regroup, or submit them as mutable members. Product destinations normally require an enabled instance. Administration, configuration, and diagnostics destinations may remain visible for an installed but disabled, unconfigured, or unhealthy instance so administrators can recover it. Hiding a contribution does not disable the module or authorize a user. Route and API enforcement remains mandatory inside the owner. In the current Sprint 6A reference composition, fixed Core Module Management is in the `Admin` group and is discoverable with effective global `modules:read`; its navigation mutation controls require `modules:manage_navigation`.
