...
| Legacy concept | Tessara target | Notes |
| --- | --- | --- |
| `ClientFormAssignment` | `workflow_assignments` via generated single-form workflows | Direct `form_assignments` storage has been removed. The Assign Form shortcut is now only a UI/import convenience: create or reuse the generated single-step workflow for the form, then create normal workflow assignments. |
