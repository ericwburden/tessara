    ...
    } = entry
    {
        ...
        return view! {
            <tr class="module-directory__row-link" data-module-definition=definition.id.clone() role="link" tabindex="0"
                ...
                <th scope="row" class="data-table__stacked-label"><span><a href=detail_href.clone()><strong>{definition.display_name.clone()}</strong></a>{if warning { Some(view! { <span title="Configuration needs review" aria-label="Configuration needs review"><TriangleAlert class="module-directory__configuration-warning"/></span> }) } else { None }}</span><code class="data-table__secondary-text">{definition.id.clone()}</code><span class="module-directory__digest"><code class="data-table__secondary-text">{compact_value(&release.manifest_digest)}</code><span class="sr-only data-table__secondary-text">{release.manifest_digest.clone()}</span><CopyValue value=release.manifest_digest label="Copy complete manifest digest"/></span></th>
